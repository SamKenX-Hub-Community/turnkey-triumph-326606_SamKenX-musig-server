...
export const putTransaction = (request, h) => {
    const { data, id }: { data: Interfaces.ITransactionData, id: string } = request.payload;
    ...
    for (const signature of storeTransaction.data.signatures) {
        if (!data.signatures.includes(signature)) {
            return Boom.badData("The transaction provided does not include all existing signatures from the transaction to update");
        }
    }
